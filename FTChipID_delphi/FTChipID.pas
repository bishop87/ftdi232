...
//uses SysUtils, DebugFile;

type FTID_STATUS = Integer;

const MAX_NUM_SERIAL_NUMBER_CHARS = 50;

type TSerialNumber = array [0..(MAX_NUM_SERIAL_NUMBER_CHARS - 1)] of ansichar;
type PSerialNumber = ^TSerialNumber;

const MAX_NUM_DESCRIPTION_CHARS = 256;

type TDescription = array [0..(MAX_NUM_DESCRIPTION_CHARS - 1)] of ansichar;
type PDescription = ^TDescription;

const MAX_NUM_DLL_VERSION_CHARS = 10;

type TDllVersion = array [0..(MAX_NUM_DLL_VERSION_CHARS - 1)] of ansichar;
type PDllVersion = ^TDllVersion;

const MAX_NUM_ERROR_MESSAGE_CHARS = 100;

type TErrorMessage = array [0..(MAX_NUM_ERROR_MESSAGE_CHARS - 1)] of ansichar;
type PErrorMessage = ^TErrorMessage;

type CFTChipID = class
 public
  constructor create;
...
function FTID_GetErrorCodeString(Language: PAnsiChar; StatusCode: FTID_STATUS; ErrorMessageBuffer: PErrorMessage; BufferSize: Longword) : FTID_STATUS; stdcall; External FT_ChipID_DLL_Name name 'FTID_GetErrorCodeString';
...
begin
  Result := FTID_GetErrorCodeString(PAnsiChar(Language), StatusCode, pErrorMessageBuffer, MAX_NUM_ERROR_MESSAGE_CHARS);
end;
